# Hi, I'm Dante García 👋

**Mechatronics Engineer** from Monterrey, México 🇲🇽 · Starting an **EU4M Erasmus Mundus MSc** @ Universidad de Oviedo 🇪🇸 & Hochschule Karlsruhe 🇩🇪 (Sept 2026)

I build things at the intersection of **robotics**, **embedded systems**, **Digital Twins**, and **AR/VR**. I thrive in international, multidisciplinary teams and am always looking for ways to apply technology to meaningful problems.
*I seek a Masters and PhD to work in R&D and make meaningful contributions to society.*

---

## 🛠️ Tech Stack

**Languages**

![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![C](https://img.shields.io/badge/C-A8B9CC?style=flat&logo=c&logoColor=black)
![C#](https://img.shields.io/badge/C%23-239120?style=flat&logo=csharp&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![MATLAB](https://img.shields.io/badge/MATLAB-0076A8?style=flat&logo=mathworks&logoColor=white)

**Tools & Platforms**

![Unity](https://img.shields.io/badge/Unity-000000?style=flat&logo=unity&logoColor=white)
![Blender](https://img.shields.io/badge/Blender-F5792A?style=flat&logo=blender&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Raspberry Pi](https://img.shields.io/badge/Raspberry_Pi-A22846?style=flat&logo=raspberrypi&logoColor=white)
![LabVIEW](https://img.shields.io/badge/LabVIEW-FFDB00?style=flat&logo=labview&logoColor=black)
![Fusion 360](https://img.shields.io/badge/Fusion_360-0696D7?style=flat&logo=autodesk&logoColor=white)

---

## 🚀 Featured Projects

### 🦿 Exoskeleton Digital Twin & Control System *(Bachelor Thesis — Cum Laude, 98%)*
Embedded control system for a bipedal exoskeleton in **C++ / RTOS**, with a real-time **Digital Twin in Unity (C#)** for medical and rehabilitation studies. Featured at the **Premio FRISA - UDEM 15th Edition** for best engineering thesis.

### 🤖 Digital Twin for Robotic Assembly Platform
Full pipeline from physical robot building to a **Unity-based Digital Twin** used to train and simulate robot behavior in an assembly line context.

### 🥽 AR Exhibition App — DAAD Project *(Karlsruhe, Germany)*
Augmented Reality application for displaying and simulating historical mechanical objects. Built with **Unity + Blender**, in collaboration with an international team from Germany, Malaysia, Tunisia, and Oman.

### 🌌 Space Experience Simulation (VR)
Immersive VR environment built with **Unity** and **Blender**, featuring version-controlled asset pipelines via Git.

### ♻️ NASA Space Apps Challenge 2025
AI-powered waste management system using the **Gemini API**, developed during the global hackathon.

---

## 🎓 Education

| Degree | Institution | Period | Grade |
|---|---|---|---|
| MSc Mechatronics Engineering — Erasmus Mundus (EU4M) | U. de Oviedo 🇪🇸 + HKA 🇩🇪 | 2026–2028 | Full EMJM Scholarship |
| BEng Mechatronics Engineering | Universidad de Monterrey (UDEM) | 2020–2025 | 98% · Cum Laude |
| Short Term Researcher | U. of Michigan SURE 🇺🇸 | 2025 | Aerospace Eng. ✈️ |
| Exchange Semester | Hochschule Karlsruhe (HKA) 🇩🇪 | 2024 | 95% |

**🏅 Ceneval EGEL+ IMecatro — Outstanding Performance (Desempeño Sobresaliente)**

---

## 🌍 Languages

🇲🇽 Spanish — Native &nbsp;|&nbsp; 🇬🇧 English — C1 &nbsp;|&nbsp; 🇩🇪 German — A2/B1 &nbsp;|&nbsp; 🇯🇵 Japanese — N5

---

## 📫 Get in touch

[![Email](https://img.shields.io/badge/Email-dante.garcia@udem.edu-0078D4?style=flat&logo=microsoftoutlook&logoColor=white)](mailto:dante.garcia@udem.edu)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/dante-antonio-garc%C3%ADa-rosales-5154832b1)


---
